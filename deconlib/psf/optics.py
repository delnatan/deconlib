...
@dataclass(frozen=True)
class OpticalConfig:
    """Legacy combined optical and grid configuration.

    Deprecated: Use separate Optics and Grid dataclasses instead.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.dx <= 0 or self.dy <= 0:
            raise ValueError(
                f"Pixel sizes must be positive, got dx={self.dx}, dy={self.dy}"
            )
    ...
    def to_optics_grid(self) -> Tuple[Optics, Grid]:
        """Convert to new Optics and Grid dataclasses."""
        optics = Optics(
